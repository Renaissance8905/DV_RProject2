...
dfquan %>% select(MAKE_NAME_NM, MODEL_NAME_NM, SYSTEM_TYPE_ETXT, UNIT_AFFECTED_NBR, YEAR) %>% group_by(MODEL_NAME_NM) %>% filter(MAKE_NAME_NM == "HONDA", SYSTEM_TYPE_ETXT != "Not Entered", SYSTEM_TYPE_ETXT != "Other") %>% ggplot(aes(x = as.numeric(as.character(YEAR)), y = as.numeric(as.character(UNIT_AFFECTED_NBR)), color = SYSTEM_TYPE_ETXT)) + geom_point()